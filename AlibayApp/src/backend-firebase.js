...
createListing
    parameters: 
      ...
      [imageFile] the File object generated by the File input element, at this.fileInput.files[0]

    returns: A promise containing the ID of the new listing
*/
function createListing(sellerID, price, title, blurb, imageFile) {
...
async function test() {
    ...
    let listing1ID = await createListing(sellerID, 500000, "boat", "A very nice boat", new File(["foo"], "foo.jpeg", {
        type: "image/jpeg",
      }))
    let listing2ID = await createListing(sellerID, 1000, "gloves", "Faux fur gloves",new File(["too"], "too.jpeg", {
        type: "image/jpeg",
      }))
