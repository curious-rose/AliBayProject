...
const serviceAccount = require("./curious-rose-key.json");
...
const itemsSellingRef = database.ref("itemsSelling") //node recording items that users are currently selling
...
function createListing(sellerID, price, title, blurb) {
    ...
    let p2 = itemsSellingRef.child(`${sellerID}/${listingID}`).set(true).catch(err=>console.log("p2 " + err))

    return Promise.all([p1, p2]).then(() => listingID).catch(err=>console.log("pall" + err))
...
function buy(buyerID, sellerID, listingID) {
    ...
    p4 = itemsSellingRef.child(`${listingID}`).remove()
...
Once an item is sold, it will not be returned by searchForListings
    ...
    returns: a promise containing an array of listing IDs
*/
function searchForListings(searchTerm, sellerID) {
    let searchString = new RegExp(`${searchTerm}`, "i")
    ...
    .then(allItemsObj => {
        ...
        (searchString.test(allItemsObj[ID].blurb) || 
        searchString.test(allItemsObj[ID].title)))
...
//bashu's tests
// allListings().then(d => console.log(d))

// getItemDescription("43989404").then(x => console.log(x))
// x = 5